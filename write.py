...
import diffuse as d
import calcnextphoton2 as calc
import time as rt
import math
import numpy
...
def write(filepath, filedir, fullfilename, antibunch, diffuse, pulsed, numlines, maxlines, endtime,
    ...
    ##Diffusion coeff D = lambda^2/2tau = uRT/zF = uK_BT where u is the mobility or the ratio of the terminal drift velocity to an applied force
    ##lambda = zuF, also u = 1/ 6 pi eta r Stokes Einstein
    ##
    ##Better: Wikipedia, Einstein relation: D = k_B T/(6 pi eta r), eta is the viscosity, r is the radius of the spherical particle
    ##
    ...
    while (line < numlines or lastwritten < endtime) and line < maxlines:
        ...
        if wrotediff != [nextIn, nextOut] and nextIn < nextOut:
            ...
            wrotediff = [nextIn, nextOut]
        #print("New Round")
        dataphotons, lastphoton, numEms, nextIn, nextOut, diffsIn, diffsOut, ndiffsOut = calc.nextphoton(lastphoton, sensitivity,
    ...
    file.write("k_emission = " + str(int(k_emission))+"\n")
    file.write("k_excitation = " +str(int(k_excitation))+"\n")
    file.write("absXsec = " + str(absXsec) + "\n")
